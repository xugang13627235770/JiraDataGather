...
export default {
  ...
  methods: {
    ...
    getSprintId(projectId){
      let dataStr = window.localStorage.getItem("tester_sel_sprint") ? window.localStorage.getItem("tester_sel_sprint") : "";
      let dataArray = [];
      if(dataStr.length > 0){
    ...
    selSprintFun(value, obj){
      ...
      window.localStorage.setItem("tester_sel_sprint",JSON.stringify(dataArray));
    },
    ...
    exportExcel() {
      ...
      this.loading = true;
      this.api
        .exportExcelTester({ sprints: sprintsList.join(",")})
        .then(res => {
          this.loading = false;
          if (res.code == 0) {
  ...
  mounted() {
    this.$nextTick(()=>{
      ...
      this.getProjectList().then(res=>{
        this.selProject = window.localStorage.getItem("tester_sel_project") ? window.localStorage.getItem("tester_sel_project").split(",") : [];
  ...
  watch:{
    selProject(newV,oldV) {
      if(newV && newV.length>0){
        window.localStorage.setItem("tester_sel_project",newV.join(","));
        console.log(window.localStorage.getItem("tester_sel_project"))
      }else{
        window.localStorage.setItem("tester_sel_project",[]);
      }
    },
  }
};
